...
.. note::
   Test databases are neutered, features are disabled to prevent them having impact on the
   production database:

   #. The serial number of the database is changed (to avoid it sending information as it was the
      production database)
   #. The base URL of the database is reset to `http://localhost:8069` and the email domain to
      `localhost`
